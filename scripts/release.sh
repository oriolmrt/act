...
echo ""
echo "File sizes:"
echo "  act.js:        $(b=$(wc -c < "$DIST_DIR/act.js" | tr -d ' '); echo "$(( b / 1024 )) KB ($b bytes)")"
echo "  act.min.js:    $(b=$(wc -c < "$DIST_DIR/act.min.js" | tr -d ' '); echo "$(( b / 1024 )) KB ($b bytes)")"
echo "  act.min.js.gz: $(b=$(wc -c < "$DIST_DIR/act.min.js.gz" | tr -d ' '); echo "$(( b / 1024 )) KB ($b bytes)")"
echo "  act.min.js.br: $(b=$(wc -c < "$DIST_DIR/act.min.js.br" | tr -d ' '); echo "$(( b / 1024 )) KB ($b bytes)")"
